...
#
# Что делает:
#   1. Копирует заготовку brain в целевую директорию.
#   2. Подставляет реальный путь brain во все файлы.
...
# подключение к Claude Code до-настраивается.
set -euo pipefail
...
[ -d "$REPO_DIR/framework" ] || die "не найдена директория framework/ рядом с install.sh — запускай скрипт из клона репозитория."
...
if [ -f "$BRAIN_DIR/CONVENTIONS.md" ]; then
  say "brain уже существует в $BRAIN_DIR — файлы не трогаю, до-настраиваю подключение к Claude Code."
  ...
  # переносимый in-place sed (GNU и BSD/macOS)
...
# --- 5. итог ---
echo
say "Готово! brain установлен в $BRAIN_DIR"
echo
echo "Дальше:"
